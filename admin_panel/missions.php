...
$result=mysql_query("INSERT INTO missions (`name`,`obiectiv`,`cantitate`,`level`,`prereq`,`mission_cost`,`special_mission`,`reward`,`reward_cantitate`,`specific_item`) VALUES ('{$_POST['name']}','{$_POST['obj']}',{$_POST['cant']},{$_POST['level']},'{$prereq}',".intval($_POST['reward_type']).",".intval($_POST['mission_cost']).",{$_POST['special_mission']},{$_POST['reward_cant']},{$_POST['specific_item']})") or die(mysql_error());
...
<select name="specific_item">

<option value="0">select</option>

<?php

$res_spec=mysql_query("SELECT * FROM items_inventory WHERE loot = 1000");

while($row_spec=mysql_fetch_array($res_spec)){

echo "<option ";

if(isset($row_edit))if($row_edit['specific_item']==$row_spec['id_item'])echo "SELECTED";

echo " value=\"{$row_spec['id_item']}\">{$row_spec['name']}</option>";
